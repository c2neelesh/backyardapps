...
class Song {
    ...
    init(songDictionaryFromSnapshot: [String : Any] ,key: String) {
        ...
        if let _ = songDictionaryFromSnapshot["songURL"] {
            ...
            FirebaseSongHandler.downloadSong(type: SongType.song,  uid: key, completion: { (songData, error) in
    ...
    func save(completion: @escaping (String?, Error?) -> Void) {
        let ref = DatabaseReference.songs.reference()
        ...
        firebaseSong.saveSong(SongType.song,  uid, { (error) in
            if error == nil {
